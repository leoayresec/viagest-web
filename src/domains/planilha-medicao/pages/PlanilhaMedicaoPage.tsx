...
import { getRecordNeighborhood, getRecordRoadName, type ViaGestRecord } from '../../../core/types/records'
...
export function PlanilhaMedicaoPage() {
  ...
  function carregarResumo() {
    const filtrados = recordsDoPeriodo(records, mes, ano).filter((r) => !bairroFiltro || getRecordNeighborhood(r) === bairroFiltro)
  ...
  function handleGerar() {
    ...
    if (geracaoTipo === 'uma_via') {
      ...
      const viaRecs = filtrados.filter((r) => getRecordNeighborhood(r) === bairroSel && getRecordRoadName(r) === viaSel)
      ...
      const bairroRecs = filtrados.filter((r) => getRecordNeighborhood(r) === bairroSel)
      ...
      const producao = filtrados.filter((r) => !bairroFiltro || getRecordNeighborhood(r) === bairroFiltro)
  ...
  const filtradosPreview = useRecordsStore((s) => {
    ...
    if (bairroSel && viaSel) return doPeriodo.filter((r) => getRecordNeighborhood(r) === bairroSel && getRecordRoadName(r) === viaSel)
    return []
  })
